...
class CldrData
{
    ...
    private static function getData($key)
    {
        if (!isset(self::$data)) {
            ...
            $data = array();
            $json = json_decode(file_get_contents(__DIR__.'/cldr-data/main/en/languages.json'), true);
            $data['languages'] = $fixKeys($json['main']['en']['localeDisplayNames']['languages']);
            $json = json_decode(file_get_contents(__DIR__.'/cldr-data/main/en/territories.json'), true);
            $data['territories'] = $fixKeys($json['main']['en']['localeDisplayNames']['territories']);
            ...
            $json = json_decode(file_get_contents(__DIR__.'/cldr-data/main/en/scripts.json'), true);
            $data['scripts'] = $fixKeys($json['main']['en']['localeDisplayNames']['scripts'], $data['standAloneScripts']);
